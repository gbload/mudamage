...
static public function getWingOp(wingname:String):Array{
	...
	if(wing[2][0])//攻撃opのBoolean
		for(i=1;i<5;i++)a.push(["Dmg opt" + (i*4),"Dmg",i*4]);
	if(wing[2][1])//魔力opのBoolean
		for(i=1;i<5;i++)a.push(["Magic option" + (i*4),"Magic",i*4]);
	if(wing[2][2])//防御opのBoolean
		for(i=1;i<5;i++)a.push(["Guard option" + (i*4),"Guard",i*4]);
	if(wing[2][3])//呪いopのBoolean
		for(i=1;i<5;i++)a.push(["Curse Opt" + (i*4),"Curse",i*4]);
	return a;
}
static public function getWingCop(wingname:String):Array{
	...
	if(wing[3] == 2){//2次羽
		a.push("Ignore +3%");
		a.push("Life Increase");
		a.push("Mana Increase");
	}else if(wing[3] == 3){//3次羽
		a.push("Ignore +5%");
		a.push("Life Completion");
		a.push("Mana Completion");
		a.push("Damage Reflection");
	}else if(wing[3] == 4){//マント
		a.push("Ignore +3%");
		a.push("Life Increase");
		a.push("Mana Increase");
		a.push("Command Increase");
	}
	return a;
}
static public function getRight(job:int):Array{//右手の一覧を返す
	...
	for(i=0;i<Weapon.weapon.length;i++)
		if(Weapon.weapon[i][5][job])//装備可能職かチェック
			if(Weapon.weapon[i][4] != "Left Hand")a.push(Weapon.weapon[i]);//持ち手を確認し、
	return a;
}
static public function getLeft(job:int):Array{//左手の一覧を返す
	...
	if(job == 0 || job == 3 || job == 5 || job == 6)//ナイト、魔剣士、召喚師のみ左手に武器を持てる
		for(i=0;i<Weapon.weapon.length;i++)
			if(Weapon.weapon[i][5][job])//装備可能職かチェック
				if(!(job == 5 && Weapon.weapon[i][3] == "Staff"))//召喚師は左手にロッドは持てない
				if(Weapon.weapon[i][4] == "One Hand" || Weapon.weapon[i][4] == "Left Hand")//持ち手を確認
...
static public function getSocketLeft(job:int):Array{//ソケット左手の一覧を返す
	...
	if(job == 0 || job == 3 || job == 5 || job == 6)//ナイト、魔剣士、召喚師のみ左手に武器を持てる
		for(i=0;i<Socket.socket_weapon.length;i++)
			if(Socket.socket_weapon[i][5][job])//装備可能職かチェック
				if(!(job == 5 && Socket.socket_weapon[i][3] == "Staff"))//召喚師は左手にロッドは持てない
				if(Socket.socket_weapon[i][4] == "One Hand" || Socket.socket_weapon[i][4] == "Left Hand")//持ち手を確認
...
static public function getSetLeft(job:int):Array{//ソケット左手の一覧を返す
	...
	if(job == 0 || job == 3 || job == 5 || job == 6)//ナイト、魔剣士、召喚師のみ左手に武器を持てる
		for(i=0;i<SetItem.set_weapon.length;i++)
			if(SetItem.set_weapon[i][5][job])//装備可能職かチェック
				if(!(job == 5 && SetItem.set_weapon[i][3] == "Staff"))//召喚師は左手にロッドは持てない
				if(SetItem.set_weapon[i][4] == "One Hand" || SetItem.set_weapon[i][4] == "Left Hand")//持ち手を確認
