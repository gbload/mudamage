...
click function InputSupportCommit(event:Event):void{
	...
	s.supportdatas[s.C_INNER].label = "Innovation:"+d.s_inner.text+"%";
	s.supportdatas[s.C_WEAK].label = "Weakness:"+d.s_weak.text+"%";
}
...
click function petChange(event:Event):void{
	...
	hide(d.f_petsub2);
	//ペットの補助フォームの作成及び表示
	if(event.target.selectedLabel == "Dinorant"){
		show(d.f_petsub1);
		show(d.f_petsub2);
		d.f_petsub1.dataProvider = ["","AG+50","Speed +5","ダメージ吸収+5%"];
		d.f_petsub2.dataProvider = ["","AG+50","Speed +5","ダメージ吸収+5%"];
	}else if(event.target.selectedLabel == "Fenrir"){
		show(d.f_petsub1);
		d.f_petsub1.dataProvider = ["Normal","Destruction","Protect","Gold"];
	}else if(event.target.selectedLabel == "Dark Horse"){
		show(d.f_petsub1);
...
click function itemChange(event:Event):void{
	...
	if(item.f_kind.selectedIndex > 0){
		...
		change::op(item);
		//380OPの変更
		if((item.f_item.selectedItem[2] == "Normal" || item.f_item.selectedItem[2] == "EXE")
...
click function jobChange(event:Event):void{
	use namespace dat;
	//ステータスの変更
	if(d.f_job.selectedLabel == "Dark Lord") d.f_rec.parent.visible = true;
	...
	//魔剣士の場合　兜を隠す
	if(d.f_job.selectedLabel == "Magic Gladiator")form::helmbox.visible = false;
	else form::helmbox.visible = true;
	//WIZの場合、スペルエンハンスを表示
	if(d.f_job.selectedLabel == "Dark Wizard")show(form::sebox);
	else{hide(form::sebox);d.s_se.selected = false;}
	//召喚師の場合、バーサーカーを表示
	if(d.f_job.selectedLabel == "Summoner")show(form::berserkerbox);
	else{hide(form::berserkerbox);d.s_berserker.selected = false;}
	//ダークロードの場合、鷹を表示する
	if(d.f_job.selectedLabel == "Dark Lord")
		d.f_left.f_kind.dataProvider = ["None","Normal","EXE","Set Ancient","Socket","EXE Socket","Dark Raver"];
	else
		d.f_left.f_kind.dataProvider = ["None","Normal","EXE","Set Ancient","Socket","EXE Socket"];
	//レイジファイターの場合、敵の防御力無視を表示
	if(d.f_job.selectedLabel == "Rage Fighter"){
		show(form::ignorebox);
...
click function mlv(event:Event):void{
	...
	if(dat::d.f_lv.text == "400"){
		...
		pop.title = "Master LV";//タイトル
		
		pop.addChild(form::mlvBox);//MLVキャンバスを載せる
	}else{
		Alert.show("Please levels 400.");
	}
}
...
click function mlvShowPoint():void{
	...
	form::mlvpoint.text = "Point " + point + "/" + dat::d.f_mlv.text;
}
...
change function statusChange():void{
	...
	if(d.f_job.selectedLabel == "Dark Lord"){
	    ...
	    var rec:int = 0;
	    if(d.f_job.selectedLabel == "Magic Gladiator"){//mの計算
	        //ポイント計算
	        point = (lv-1)*7;
	        if(lv>=380)point += 20;
	        if(lv==400)point += 50;
	    ...
	    }else if(d.f_job.selectedLabel == "Dark Lord"){//dの計算
	        //ポイント計算
	        point = (lv-1)*7;
	        if(lv>=380)point += 20;
	        if(lv==400)point += 50;
