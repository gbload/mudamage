package AAgetter
{
	...
	public class InputForm
	{
			...
			canvas.addChild(h);
				...
				h.addChild(vbox);
					...
					vbox.addChild(hbox);
						...
						//攻撃速度
						speed = createNumberInput("Speed",hbox,3);
						//攻撃速度(ms)
						speedms = createNumberInput("Speed(ms)",hbox,4);
						//ディレイ(ms)
						delay = createNumberInput("Delay(ms)",hbox,4);
					//2段目
					hbox = new HBox();
					vbox.addChild(hbox);
						//最小ダメージ
						min = createNumberInput("Min",hbox,5);
						//最大ダメージ
						max = createNumberInput("Max",hbox,5);
						//クリティカルダメージ
						cri = createNumberInput("Critical",hbox,5);
						//EXDダメージ
						exd = createNumberInput("XDMG",hbox,5);
						//クリティカル確率
						criper = createNumberInput("Critical Rate",hbox,3,true);
						//EXD確率
						exdper = createNumberInput("XDMG Rate",hbox,3,true);
						//ダブルダメージ確率
						wdper = createNumberInput("WD Rate",hbox,3,true);
		}
		//入力フォーム
		...
		public function setResult(id:InputData):void{
			...
			var str:String = "Attack Count" + (Math.floor(hit*10/attempts)/10).toString() + "回\n";
			str += "Average" + (Math.floor(damage*10/attempts)/10).toString() + "ダメージ\n";
			str += "Win Count" + win + "回\n";
			str += "Win Rate" + (Math.floor(win*1000/attempts)/10).toString() + "%";
			result.text = str;
		}
