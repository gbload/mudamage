...
calcchar function Pet(muc:MuChar):MuChar{
	...
	muc.satan = satan;//サタン
	muc.angel = angel;//天使
	muc.deamon = (dat::d.f_pet.selectedLabel == "Demon");
	muc.spirit = (dat::d.f_pet.selectedLabel == "Spirit of Guardian");
	muc.uniria = (dat::d.f_pet.selectedLabel == "Uniria");
	if(dat::d.f_pet.selectedLabel == "Dinorant"){
		muc.dinolunt = [1,0,0];//[有無,op1,op2]
		if(dat::d.f_petsub1.selectedLabel == "AG+50")muc.dinolunt[1]=1;
		if(dat::d.f_petsub1.selectedLabel == "Speed +5")muc.dinolunt[1]=2;
		if(dat::d.f_petsub1.selectedLabel == "Damage Decrease+5")muc.dinolunt[1]=3;
		if(dat::d.f_petsub2.selectedLabel == "AG+50")muc.dinolunt[2]=1;
		if(dat::d.f_petsub2.selectedLabel == "Speed +5")muc.dinolunt[2]=2;
		if(dat::d.f_petsub2.selectedLabel == "Damage Decrease+5")muc.dinolunt[2]=3;
	}
	if(dat::d.f_pet.selectedLabel == "Fenrir"){
		if(dat::d.f_petsub1.selectedLabel == "Normal")muc.fenrir=1;
		if(dat::d.f_petsub1.selectedLabel == "Destruction")muc.fenrir=2;
		if(dat::d.f_petsub1.selectedLabel == "Protect")muc.fenrir=3;
		if(dat::d.f_petsub1.selectedLabel == "Gold")muc.fenrir=4;
	}
	if(dat::d.f_pet.selectedLabel == "Dark Horse")
		muc.darkhorse = dat::d.f_petsub1.selectedIndex + 1;//LVを代入
	muc.ponga = (dat::d.f_pet.selectedLabel == "Pet Panda");
	muc.arion = (dat::d.f_pet.selectedLabel == "Pet Unicorn");
	muc.skelton = (dat::d.f_pet.selectedLabel == "Pet Skeleton");
	return muc;
}
calcchar function Support(muc:MuChar):MuChar{
	use namespace calc;
	if(dat::d.f_job.selectedLabel=="Dark Knight")muc.support_a = Math.floor(support_a * 1.1);//A+
	else muc.support_a = support_a;//A+
	if(dat::d.f_job.selectedLabel=="Dark Knight")muc.support_g = Math.floor(support_g * 1.1);//G+
...
calcchar function Status(muc:MuChar):MuChar{
	...
	muc.lefthund = (dat::d.f_left.f_kind.selectedLabel != "None" && dat::d.f_left.f_item.selectedItem[0] == "Weapon");
...
calcchar function Def(muc:MuChar):MuChar{
	...
	if(d.f_job.selectedLabel == "Dark Knight"){
		de += muc.agi/3;
		avoid = muc.agi/3;
		pvpavoid = lv*2 + muc.agi/2;
	}else if(d.f_job.selectedLabel == "Dark Wizard"){
		de += muc.agi/4
		avoid = muc.agi/3;
		pvpavoid = lv*2 + muc.agi/4;
	}else if(d.f_job.selectedLabel == "Fairy Elf"){
		de += muc.agi/10;
		avoid = muc.agi/4;
		pvpavoid = lv*2 + muc.agi/10;
	}else if(d.f_job.selectedLabel == "Magic Gladiator"){
		de += muc.agi/4;
		avoid = muc.agi/3;
		pvpavoid = lv*2 + muc.agi/4;
	}else if(d.f_job.selectedLabel == "Dark Lord"){
		de += muc.agi/7;
		avoid = muc.agi/7;
		pvpavoid = lv*2 + muc.agi/2;
	}else if(d.f_job.selectedLabel == "Summoner"){
		de += muc.agi/3;
		avoid = muc.agi/4;
		pvpavoid = lv*2 + muc.agi/2;
	}else if(d.f_job.selectedLabel == "Rage Fighter"){
		de += muc.agi/8;
		avoid = muc.agi/10;
	...
	for each(var item:Item in a)
		if(item.f_kind.selectedLabel != "None"){
			//380OPの対人防御成功率加算
			if(item.f_380.visible && item.f_380.selected)
				pvpavoid += 10;//380OP 
			//DEFを加算
			if(item.f_kind.selectedLabel == "てすちゅ")
	...
	if(d.f_left.f_kind.selectedLabel != "None")
		if(d.f_left.f_item.selectedItem[0] == "Protect"){//盾
	...
	//防御成功率====================================================================
	if(d.f_left.f_kind.selectedLabel != "None" && d.f_left.f_item.selectedItem[0] == "Protect"){
		if(d.f_left.f_kind.selectedLabel == "てすちゅ")//盾から取得
...
calcchar function Speed(muc:MuChar):MuChar{
	...
	if(d.f_left.f_kind.selectedLabel != "None" && d.f_left.f_item.dataProvider != "" && d.f_left.f_kind.selectedLabel != "Dark Raver"){
	...
	//グラブの攻撃速度
	if(d.f_glove.f_kind.selectedLabel != "None"){
...
calcchar function Normal(muc:MuChar):MuChar{
	...
	//クリ
	var cri:int = 0;
	if(d.f_wing.f_item.selectedLabel != "None" && d.f_wing.f_luck.selected)cri += 4;
	if(d.f_right.f_kind.selectedLabel != "None" && d.f_right.f_luck.selected)cri += 4;
	if(d.f_left.f_kind.selectedLabel != "None" && d.f_left.f_luck.selected)cri += 4;
	if(d.f_helm.f_kind.selectedLabel != "None" && d.f_helm.f_luck.selected)cri += 4;
	if(d.f_armor.f_kind.selectedLabel != "None" && d.f_armor.f_luck.selected)cri += 4;
	if(d.f_glove.f_kind.selectedLabel != "None" && d.f_glove.f_luck.selected)cri += 4;
	if(d.f_garter.f_kind.selectedLabel != "None" && d.f_garter.f_luck.selected)cri += 4;
	if(d.f_boots.f_kind.selectedLabel != "None" && d.f_boots.f_luck.selected)cri += 4;
	...
	for each(var i:Item in a){
		if(i.f_kind.selectedLabel == "Socket" || i.f_kind.selectedLabel == "EXE Socket"){
			for each(var j:ComboBox in i.f_soop){
				//ボーナスソケットの確認
				if(j.selectedIndex <= 0)continue;
				if(j.selectedItem[1] == "Fire")att[0]++;
				if(j.selectedItem[1] == "Lightning")att[1]++;
				if(j.selectedItem[1] == "Ice")att[2]++;
				if(j.selectedItem[1] == "Water")att[3]++;
				if(j.selectedItem[1] == "Wind")att[4]++;
				if(j.selectedItem[1] == "Earth")att[5]++;
			}
		}
	}
...
calcchar function Skill(muc:MuChar):MuChar{
		...
		if(i[5][muc.job])//職確認
			if(i[4][0] <= muc.lv && i[4][1] <= muc.str && i[4][2] <= muc.agi
				&& i[4][3] <= muc.ene && i[4][4] <= muc.rec){//要求ステ確認
					...
					//ペット
					if(i[6]){
						if(i[6] == "Dinorant" && !muc.dinolunt[0])continue;//ディノラントの確認
						if(i[6] == "Fenrir" && !muc.fenrir)continue;//フェンリルの確認
						if(i[6] == "Dark Horse" && !muc.darkhorse)continue;//ダークホースの確認
					}else if(i[7]){//武器S
						if(!((dat::d.f_right.f_kind.selectedLabel != "None"//右手の有無
							&& i[0] == dat::d.f_right.f_item.selectedItem[8])//スキル名が一致するか
							||(dat::d.f_left.f_kind.selectedLabel != "None"//左手の有無
							&& dat::d.f_left.f_item.selectedItem[0] == "Weapon"//左手が武器であるか
							&& i[0] == dat::d.f_left.f_item.selectedItem[8])))//スキル名が一致するか
							continue;
					}else if(i[9] == 4){//ダークスピリット
						if(dat::d.f_left.f_kind.selectedLabel != "Dark Raver")continue;//鷹の有無
					}
					//スキルを追加
					//属性ダメージ計算
					attribute = 0;//初期化
					if(dat::d.f_neck.f_kind.selectedLabel == "Set Ancient"
						&& dat::d.f_neck.f_item.selectedItem[8] == i[8])//セットアイテム8番目の配列に属性、スキル8番目の配列に属性
						attribute +=5;//属性ダメージ＋5
					if(dat::d.f_ring1.f_kind.selectedLabel == "Set Ancient"
						&& dat::d.f_ring1.f_item.selectedItem[8] == i[8])//セットアイテム8番目の配列に属性、スキル8番目の配列に属性
						attribute +=5;//属性ダメージ＋5
					if(dat::d.f_ring2.f_kind.selectedLabel == "Set Ancient"
	...
	for each(i in muc.skill){
		...
		for(var k:int=sp.length - 1;k>=0;k--)
			if((i[3] == "Skill" && muc.speed >= sp[k][0]) || (i[3] == "Magic" && muc.magicspeed >= sp[k][0])){
				i[2] = [sp[k][1],sp[k][2]];
				check=false;
				break;
			}
	...
	//スキル強化
	var a:Array = [["Twisting Slash",dat::mlvcount.tc,MLV.inf_tc]
					,["Death Stab",dat::mlvcount.gb,MLV.inf_gb]
					,["Earth Break",dat::mlvcount.oi,MLV.inf_oi]
					,["Hellfire",dat::mlvcount.hell_fire,MLV.inf_hell_fire]
					,["Evil Spirits",dat::mlvcount.soul_the_end,MLV.inf_soul_the_end]
					,["Evil Spirits",dat::mlvcount.soul_the_end_ma,MLV.inf_soul_the_end_ma]
					,["Ice Storm",dat::mlvcount.sudden_ice,MLV.inf_sudden_ice]
					,["Triple Shot",dat::mlvcount.midareuti,MLV.inf_midareuti]
					,["Power Slash",dat::mlvcount.ps,MLV.inf_ps]
					,["Fire Slash",dat::mlvcount.ba,MLV.inf_ba]
					,["Cometfall",dat::mlvcount.shining_blast,MLV.inf_shining_blast]
					,["Twisting Slash",dat::mlvcount.tc_ma,MLV.inf_tc_ma]
					,["Earthquake",dat::mlvcount.earth_shake,MLV.inf_earth_shake]
					,["Fire Burst",dat::mlvcount.fb,MLV.inf_fb]
					,["Fire Scream",dat::mlvcount.fs,MLV.inf_fs]
					,["Chain Lightning",dat::mlvcount.chain,MLV.inf_chain]
					,["Drain Life",dat::mlvcount.life,MLV.inf_life]
					,["Lightning Shock",dat::mlvcount.light,MLV.inf_light]];
	for each(i in muc.skill)
...
calcchar function Option(muc:MuChar):MuChar{
    ...
    if(dat::d.f_left.f_380.visible && dat::d.f_left.f_380.selected)
    	muc.op_pvpdamage += 200;
    //両手武器装備時ダメージ増加
    if(dat::d.f_right.f_kind.selectedLabel != "None"
    	&& dat::d.f_right.f_item.selectedItem[4] == "Two-handed")
...
calcchar function MinMax2(muc:MuChar):MuChar{
    ...
    for(var i:int=0;i<muc.skill.length;i++){
    	...
    	if(muc.now_skill[0] == "Plasma Storm"){
	    		//最小
...
calcchar function calcMinMax(muc:MuChar,cri:Boolean,exd:Boolean,min:Boolean):int{
    ...
    if(muc.job == 2 && dat::d.f_right.f_kind.selectedLabel != "None"
    	&& dat::d.f_right.f_item.selectedItem[3] == "Bow"
    	&& dat::d.f_right.f_item.selectedItem[4] == "One Hand")
    		bowcheck = false;
	
    //最小最大攻撃力=================================================================
	if(dat::d.f_job.selectedLabel == "Dark Knight"){
	    if(min)d = Math.floor(muc.str/6);//Ｒ ＝ ステータス最小攻撃力
	    else d = Math.floor(muc.str/4);//Ｒ ＝ ステータス最大攻撃力
	}else if(dat::d.f_job.selectedLabel == "Dark Wizard"){
	    if(min)d = Math.floor(muc.str/8);//Ｒ ＝ ステータス最小攻撃力
	    else d = Math.floor(muc.str/4);//Ｒ ＝ ステータス最大攻撃力
	}else if(dat::d.f_job.selectedLabel == "Fairy Elf"){
	...
	}else if(dat::d.f_job.selectedLabel == "Magic Gladiator"){
	...
	}else if(dat::d.f_job.selectedLabel == "Dark Lord"){
	...
	}else if(dat::d.f_job.selectedLabel == "Summoner"){
	    if(min)d = Math.floor((muc.str-1)/9);//Ｒ ＝ ステータス最小攻撃力
	    else d = Math.floor((muc.str-1)/4);//Ｒ ＝ ステータス最大攻撃力
	}else if(dat::d.f_job.selectedLabel == "Rage Fighter"){
...
calcchar function calcMinMax2(d:int,muc:MuChar,cri:Boolean,exd:Boolean,min:Boolean):int{
	...
	//弓の場合、マルチショット特殊減少
	if(!bowcheck && muc.now_skill[0] == "Multi-Shot"){
...
calcchar function calcDarkSpirit(muc:MuChar):MuChar{
	if(dat::d.f_left.f_kind.selectedLabel == "Dark Raver"){
