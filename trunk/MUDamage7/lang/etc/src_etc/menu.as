...
menu function save(event:Event):void{
	...
	te.text = "Save the following titles.";
...
menu function savedelete(event:Event):Boolean{
	...
	if(ch == true){
		Alert.show("[" + title + "]" + "will remove."
					,"Confirm",(Alert.YES | Alert.NO),null,menu::savedelete);
		return false;
	}else{
		Alert.show("Please select the data to delete from the list above.");
		return false;
	}
	return true;
}
	...
	if(event.target.label == "Save"){
		...
		if(ch == true){
			Alert.show("Data for the title already exists. Overwrite?"
						,"Confirm",(Alert.YES | Alert.NO),null,menu::savebutton);
			return false;
		}
	}
	//保存
...
menu function savedata(title:String):void{
	...
	try{
		...
		Alert.show("Error:Failed to store data."+flushStatus);
	}
    if (flushStatus != null) {
        switch (flushStatus) {
            case SharedObjectFlushStatus.PENDING:
                Alert.show("Please allow storage of data.");
                break;
...
menu function open(event:Event):Boolean{
	...
	var te:Text = new Text();//案内
	te.text = "Open the following title.";
	te.setStyle("fontWeight","bold");
	f.addChild(te);
	...
	f.addChild(list);
	if(i == 0){
		Alert.show("There is no data being saved.");
		return false;
	}
...
menu function opendata(title:String):Boolean{
	...
	if(!ch){
		Alert.show("Could not find the specified data of the title.");
		return false;
	}
	menu::setdata(a);
	Alert.show("Loaded data.");
	return true;
}
...
menu function exportbutton(event:Event):Boolean{
	...
	if(event.target.label == "All Select"){
