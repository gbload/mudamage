...
calc function AACalcDamage():void{
	...
	//速度
	if(id.skill == "Chain Lightning")id.speed = muc.magicspeed;
	else id.speed = muc.speed;
	//速度ms
	id.speedms = muc.now_skill[2][0];
	//ディレイ
	if(id.skill == "Death Stab")id.delay = 200;
	else if(id.skill == "Multi-Shot")id.delay = 100;
	else if(id.skill == "Fire Scream")id.delay = 300;
	else if(id.skill == "Chaotic Diseier")id.delay = 300;
	else if(id.skill == "Fire Slash")id.delay = 300;
	else if(id.skill == "Chain Lightning")id.delay = 200;
	//ダメージ
	id.min = x_min;
	id.max = x_max;
	id.cri = x_cri;
