...
calcform function showHTML1():MuChar{
	...
	hdat += '<ul> ';
	//ペット
	if(d.f_pet.selectedLabel != "なし")hdat += '<li>'+ d.f_pet.selectedLabel +'</li> ';;
	//羽
	...
	for(i = 0;i < ia.length;i++){
		hdat += '<li>' + iname[i] +'　';
		if(ia[i].f_kind.selectedLabel != "なし"){
			//"EX"の表示
			...
			if(ia[i].f_item.selectedItem[0] == "武器"){
				if(spec[0] > 0 || spec[1] > 0)specstr += "攻撃力 " + spec[0] + "～" + spec[1] + "<br>";
				if(spec[5] > 0)specstr += "魔力 " + spec[5] + "%<br>";
				if(spec[6] > 0)specstr += "ペット " + spec[6] + "%<br>";
				if(spec[7] > 0)specstr += "呪い " + spec[7] + "%<br>";
				specstr += "速度 " + specitem[7]+ "<br>";
				if(spec[2] > 0)specstr += "要求力 " + (spec[2] + (ia[i].f_op.selectedIndex * 5)) + "<br>";
				if(spec[3] > 0)specstr += "要求敏捷 " + spec[3] + "<br>";
				if(spec[4] > 0)specstr += "要求エナジー " + spec[4] + "<br>";
				if(specitem[6] > 0)specstr += "要求LV " + specitem[6] + "<br>";
			}else{
				if(spec[3] > 0)specstr += "防御力 " + spec[3]+ "<br>";
				if(spec[4] > 0)specstr += "防御成功率 " + spec[4] + "<br>";
				if(specitem[7] > 0)specstr += "速度 " + specitem[7] + "<br>";
				if(spec[0] > 0)specstr += "要求力 " + (spec[0] + (ia[i].f_op.selectedIndex * 5)) + "<br>";
				if(spec[1] > 0)specstr += "要求敏捷 " + spec[1] + "<br>";
				if(spec[2] > 0)specstr += "要求エナジー " + spec[2] + "<br>";
				if(specitem[6] > 0)specstr += "要求LV " + specitem[6] + "<br>";
			}
			//アイテム名　+13 op16
