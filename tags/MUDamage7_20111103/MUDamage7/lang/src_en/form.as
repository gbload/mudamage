...
form function init():void{
	...
	b.height = 20;
	b.label = "All Export/Import";
	hbox.addChild(b);
	
	//石造破壊シミュレータ
	b = new Button();
	b.height = 20;
	b.label = "Blood Castle AA Getter Simulator";
	b.name = "0";
	...
	*/
	...
	//右手フォームの作成
	f.addChild(formEquip("Right:",d.f_right,d.f_right.name));
	
	//左手フォームの作成
	f.addChild(formEquip("Left:",d.f_left,d.f_left.name));
	
	//兜フォームの作成
	form::helmbox = formEquip("Helm:",d.f_helm,d.f_helm.name);
	f.addChild(form::helmbox);
	
	//鎧フォームの作成
	f.addChild(formEquip("Armor:",d.f_armor,d.f_armor.name));
	
	//手フォームの作成
	form::glovebox = formEquip("Gloves:",d.f_glove,d.f_glove.name);
	f.addChild(form::glovebox);
	
	//腰フォームの作成
	f.addChild(formEquip("Pants:",d.f_garter,d.f_garter.name));
	
	//足フォームの作成
	f.addChild(formEquip("Boots:",d.f_boots,d.f_boots.name));
	
	//リング１のフォームの作成
	f.addChild(formRing("Ring1",d.f_ring1,d.f_ring1.name));
	
	//リング２のフォームの作成
	f.addChild(formRing("Ring2",d.f_ring2,d.f_ring2.name));
...
private function formEquip(name:String,item:Item,f_name:String):FormItem{
	...
	var data:ArrayCollection = new ArrayCollection(["None","Normal","EXE","Set Ancient","Socket","EXE Socket"]);
...
private function formRing(name:String,item:Item,f_name:String):FormItem{
	...
	kind.name = f_name;
	kind.addEventListener(ListEvent.CHANGE,click::kindChangeAcc);
	var data:ArrayCollection = new ArrayCollection(["None","EXE","Set Ancient","Wizard","Critical Ring Wizard's","Panda Ring","Skeleton Ring","Shop"]);
	kind.dataProvider = data;
	kind.rowCount = 6;
	fi.addChild(kind);
...
private function formStatus():Form{
	...
	//各種ステータスフォームの作成
	var stform:FormItem;
	var l:Array = ["Strength:","Agility:","Vitality:","Energy:","Command:"]
	for(var i:int = 0;i<5;i++){
		stform = new FormItem();
		stform.label = l[i];
...
private function formPet():FormItem{
	...
	dat::d.f_pet = co;
	co.dataProvider = new ArrayCollection(
		["None","Imp","Guardian Angel","Demon","Spirit of Guardian"
		,"Uniria","Dinorant","Fenrir"
		,"Pet Panda","Pet Unicorn","Pet Skeleton"]);
	co.rowCount = 20;
...
private function formSupport():Container{
	...
	vbox.addChild(hbox);
		...
		box.addChild(ch);
		la = new Label();
		la.text = "Defense Success Rate Increase";
		box.addChild(la);
		hbox.addChild(box);
		//インナーベーション
		fi = new FormItem();
	...
	vbox.addChild(hbox);
		...
		hbox.addChild(ch);
		la = new Label();
		la.text = "Miracle Potion";
		hbox.addChild(la);
		//サンタ・花見・ジャックランタン
		var co:ComboBox = new ComboBox();
		d.s_item = co;
		co.dataProvider = ["None","Speed +10","Dmg +25","Defence +100","Life +500","Mana +500"
								,"Dmg +40","Life +700","Mana +700"
								,"Speed +15","Dmg +30","AG Increase Rate +10"];
		co.rowCount = 20;
		hbox.addChild(co);
		//課金スクロール
		co = new ComboBox();
		d.s_scroll = co;
		co.dataProvider = ["none scroll","Speed +15","Dmg +30","Magic +30"
							,"Defence +60","Life +200","Mana +300","Critical Dmg +20%","XDMG +20%"];
		co.rowCount = 10;
		hbox.addChild(co);
		//課金秘薬
		co = new ComboBox();
		d.s_hiyaku = co;
		co.dataProvider = ["None Miracle Potion","Strength +50","Agility +50","Vitality +50","ene+50","Command +50"];
		co.rowCount = 10;
		hbox.addChild(co);
...
private function formInputSupport():void{
	...
	ta.text = "備考：※A+の要求エナ92\n" + 
			"Greater Defense and Damage is untested in Master Level";
	ta.percentWidth = 100;
	v.addChild(ta);
	var vbox:VBox = new VBox();
	hbox.addChild(vbox);
		...
		var co:ComboBox = new ComboBox();
		vbox.addChild(co);
		co.dataProvider = ["A+MLv0[(ene/7)+3]","A+MLv1[(ene/6.7)+6]","A+MLv2[(ene/6.2)+9]"
							,"A+MLv3[(ene/5.9)+12]","A+MLv4[(ene/5.6)+15]","A+MLv5[(ene/5.3)+18]"];
		co.selectedIndex = 0;
		co.rowCount = 10;
		co.name = "mlv_a";
		...
		co = new ComboBox();
		vbox.addChild(co);
		co.dataProvider = ["G+MLv0[(ene/8)+2]","G+MLv1[(ene/7.6)+4]","G+MLv2[(ene/7.2)+6]"
							,"G+MLv3[(ene/6.8)+8]","G+MLv4[(ene/6.4)+10]","G+MLv5[(ene/6)+12]"];
		co.selectedIndex = 0;
		co.rowCount = 10;
		co.name = "mlv_g";
	...
	v.addChild(hbox);
	ta = new TextArea();
	ta.text = "Note:SL = (vit/100)+(ene/20)+([PT member]-1)";
	ta.percentWidth = 100;
	ta.height = 20;
	v.addChild(ta);
	vbox = new VBox();
	...
	hbox.addChild(vbox);
		...
		co = new ComboBox();
		vbox.addChild(co);
		co.dataProvider = ["PT:1 person","PT:2 person","PT:3 person","PT:4 person","PT:5 person"];
		co.selectedIndex = 0;
		co.rowCount = 10;
		co.name = "ptm";
	...
	v.addChild(hbox);
	ta = new TextArea();
	ta.text = "note:SB = (agi/50)+(ene/200)";
	ta.percentWidth = 100;
	ta.height = 20;
	v.addChild(ta);
	vbox = new VBox();
...
private function formMLV():void{
	...
	form::mlvpoint.text = "Point 0/0";
	hbox.addChild(form::mlvpoint);
	
	//ツリーの準備
	var treeTitle:Array = [	["Protection","Peace","Divine","Strong","Firm","Protect"],
							["Fame","Wisdom","Relief","Warrior","Justice","Chaos"],
							["Rage","Over","Gale","Ultimate","Conquest","Honor"],
							["Hero","Mystery","Belief","Win","Glory","Trust"]];
...
private function formPVPCalc():Container{
	...
	var button:Button = new Button();
	hbox.addChild(button);
	button.label = "PVP calc(alpha ver)";
