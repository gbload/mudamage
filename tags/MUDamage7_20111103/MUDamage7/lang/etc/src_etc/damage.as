...
damage function table(muc:MuChar,canvas:Container):void{
	...
	var dgc:DataGridColumn = new DataGridColumn();
		...
		dgc.headerText = "Skill Name";
		dga.push(dgc);
		dgc.width = 150;
	dgc = new DataGridColumn();
		dgc.dataField = "hitper";
		dgc.headerText = "1Hit Dmg/sec";
		dga.push(dgc);
		dgc.width = 150;
	dgc = new DataGridColumn();
		dgc.dataField = "speed";
		dgc.headerText = "count/min";
		dgc.itemRenderer = new ClassFactory(itemRenderer01);
		dga.push(dgc);
	dgc = new DataGridColumn();
		dgc.dataField = "even";	
		dgc.headerText = "1Hit Average";
		dga.push(dgc);
	dgc = new DataGridColumn();
		dgc.dataField = "minmax";
		dgc.headerText = "Min - Max";
		dga.push(dgc);
		dgc.width = 150;
	dgc = new DataGridColumn();
		dgc.dataField = "cri";
		dgc.headerText = "Critical";
		dga.push(dgc);
	dgc = new DataGridColumn();
		dgc.dataField = "exd";
		dgc.headerText = "XDMG";
	...
	for(var j:int = 0;j<muc.skill.length;j++){
		...
		if(muc.skill[j][5] == 4){//ダークスピリット
			if(muc.skill[j][0] == "Raven Attack(Single)"){
			...
			x_cri=x_max;
			x_exd=0;
		}else if(muc.skill[j][3] == "Magic"){//魔法
		...
		if(muc.skill[j][0] == "Raven Attack(Single)"){//ダークスピリット
			hit1 += x_max * muc.darkspirit[5]/100;//クリティカル率
			hit1 += ((x_min + x_max)/2) * (100 - muc.darkspirit[5]) / 100;//通常
		}else if(muc.skill[j][0] == "Raven Attack(All)"){//フレイムハンド範囲
			//クリティカル確率0%
	...
	for(var re:int = 0;re < mon.length;re++){
		...
		if(d.f_map.selectedLabel.match("Blood Castle") != null){
			// ブラッドキャッスルの場合、経験値半減
			minexp /= 2;
			maxexp /= 2;
		}
		...
		//モンスター情報を表示
		mtext = new Label();
		mtext.text = m[1] + " Lv" + m[2] + " HP" + m[3] + " Damage" + m[4] + "-" + m[5]
						+ "(rate" + m[8] + ") Defence" + m[6] + "(rate" + m[7] + ")"
						+ " Exp" + minexp + "-" + maxexp;
		canvas.addChild(mtext);
		...
		x_cri = 0;
		x_exd = 0;
		lefthund = (dat::d.f_left.f_kind.selectedLabel != "None" && dat::d.f_left.f_item.selectedItem[0] == "Weapon");
...
damage function calcDamage(muc:MuChar,d:int,elf:Boolean=false,cri:Boolean=false,exd:Boolean=false):int{
	...
	*/
    ...
    if(muc.now_skill[0] == "Plasma Storm"){
    	...
    	if(muc.now_skill[5] == 10)//武器スキル、チェーンドライブ
...
//    		d += Math.floor(d * Math.floor((120 + muc.vit/10))/100);//スキル%
    ...
    d += muc.op_damage;
    //コンボスキル
    if(muc.now_skill[5] == 7){
    	d += Math.floor((muc.str + muc.agi + muc.ene)/2);
    }
