	...
	public class Calculation
	{
		...
		private function initCalc(a:Array):void{
			...
			for(var i:int=0;i<a.length;i++){
				...
				//タイミングのずれを設定
				if((a[i] as InputData).skill != "ゴッドブロー"
					&& (a[i] as InputData).skill != "チェーンライトニング")
		...
		private function getHit(skill:String):int{
			...
			}else if(skill == "マルチショット")
				hit = 5;
			else if(skill == "ファイアスクリーム")
				hit = 3;
			else if(skill == "カオティックディセイアー")
				hit = 2;
			else if(skill == "ブラッドアタック")
				hit = 3;
			else if(skill == "チェーンライトニング")
				hit = 3;
			else
				Alert.show("不明なスキル名です。");
